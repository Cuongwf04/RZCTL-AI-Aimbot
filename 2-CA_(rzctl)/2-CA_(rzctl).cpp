...
void initializeInputMethod()
{
    ...
    try {
        rzctl = new RZControl(L"rzctl.dll");
        if (!rzctl->initialize()) {
            std::cerr << "[RZControl] Failed to initialize RZControl device." << std::endl;
            delete rzctl;
            rzctl = nullptr;
        }
    }
    ...
    globalMouseThread->setRZControl(rzctl);
}
...
void handleEasyNoRecoil(MouseThread& mouseThread)
{
    if (config.easynorecoil && shooting.load() && zooming.load())
    {
        ...
        if (rzctl)
        {
            rzctl->moveMouse(0, recoil_compensation, false);
        }
    }
}
