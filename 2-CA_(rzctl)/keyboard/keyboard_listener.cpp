...
bool is_targeting_active() {
    return (config.input_method == "WIN32" && GetAsyncKeyState(VK_RBUTTON) & 0x8000) ||
           (config.input_method == "RZCONTROL" && GetAsyncKeyState(VK_RBUTTON) & 0x8000);
}

bool is_shooting_active() {
    return (config.input_method == "WIN32" && GetAsyncKeyState(VK_LBUTTON) & 0x8000) ||
           (config.input_method == "RZCONTROL" && GetAsyncKeyState(VK_LBUTTON) & 0x8000);
}

bool is_zooming_active() {
    return (config.input_method == "WIN32" && GetAsyncKeyState(VK_RBUTTON) & 0x8000) ||
           (config.input_method == "RZCONTROL" && GetAsyncKeyState(VK_RBUTTON) & 0x8000);
}
