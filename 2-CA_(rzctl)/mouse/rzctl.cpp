...
int main() {
    try {
        RZControl rz(L"path_to_your_dll.dll");
        rz.initialize();

        // Single movement
        rz.moveMouse(1000, 1000, false);

        // Multiple movement instructions
        std::vector<std::pair<int, int>> movements = {
            {1000, 1000},  // Move to x:1000, y:1000
            {2000, 2000},  // Move to x:2000, y:2000
            {1500, 1500}   // Move to x:1500, y:1500
        };
        
        // Process movements with 100ms delay between each
        rz.processMoveInstructions(movements, false, 100);

        // Example with relative movements
        std::vector<std::pair<int, int>> relative_moves = {
            {100, 100},    // Move +100x, +100y from current
            {-50, 200},    // Move -50x, +200y from current
            {0, -150}      // Move 0x, -150y from current
        };
        
        rz.processMoveInstructions(relative_moves, true, 50);

        // Keyboard and click examples
        rz.sendKeyboardInput(30, KeyboardInputType::KEYBOARD_DOWN);
        rz.sendKeyboardInput(30, KeyboardInputType::KEYBOARD_UP);
        rz.mouseClick(MouseClick::LEFT_DOWN);
        rz.mouseClick(MouseClick::LEFT_UP);
    }
